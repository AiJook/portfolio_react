...
export function Hero() {
    ...
    useEffect(() => {
        // เนื่องจากโปรเจกต์นี้ไม่มี Backend ผมจึงจำลองระบบเก็บข้อมูลด้วย LocalStorage
        // ถ้าเอาไปใช้จริง สามารถต่อ API (เช่น Firebase, Supabase) ตรงนี้ได้เลยครับ
        const count = localStorage.getItem("cv_download_count");
        if (count) {
            setDownloadCount(parseInt(count, 10));
        } else {
            // เซ็ตค่าเริ่มต้นให้ดูน่าสนใจ
            setDownloadCount(142);
            localStorage.setItem("cv_download_count", "142");
        }
    }, []);
                    ...
                    <div className="flex flex-col gap-2 pt-4">
                        <div className="flex flex-wrap gap-4">
                            ...
                            <div className="relative group/dl">
                                <a
                                    href="/Software_Developer_Sutthikan_Butnangkul.pdf"
                                    download="Sutthikan_Resume.pdf"
                                    onClick={handleDownload}
                                    className="px-6 py-3 h-full rounded-xl border border-white/10 bg-white/5 text-white font-semibold flex items-center gap-2 hover:bg-white/10 transition-all hover:-translate-y-1 relative z-10"
                                >
                                    <Download className="size-4" />
                                    Download CV
                                </a>
                                {/* Badge count indicator */}
                                <div className="absolute -top-3 -right-3 px-2 py-0.5 rounded-full bg-secondary text-white text-[10px] font-bold shadow-[0_0_10px_rgba(14,165,233,0.5)] border border-white/20 group-hover/dl:scale-110 transition-transform z-20">
                                    {downloadCount}
                                </div>
                            </div>
                        </div>
                        <p className="text-xs text-slate-500 mt-2 pl-2 flex items-center gap-2">
                ...
                <div className="hidden lg:flex justify-center relative items-center min-h-[500px]">
                    <div className="relative w-full aspect-square max-w-[450px] flex items-center justify-center">
                        ...
                        <div className="absolute inset-0 rounded-full border border-secondary/20 animate-[spin_25s_linear_infinite] hover:[animation-play-state:paused]">
                            {/* Docker/Box */}
                            <div className="absolute -top-5 left-1/2 -translate-x-1/2 w-10 h-10 bg-[#050505] rounded-full border border-secondary/50 flex items-center justify-center shadow-[0_0_20px_rgba(14,165,233,0.5)] animate-[spin_25s_linear_infinite_reverse]">
                            ...
                            {/* Cloud */}
                            <div className="absolute -bottom-5 left-1/2 -translate-x-1/2 w-10 h-10 bg-[#050505] rounded-full border border-secondary/50 flex items-center justify-center shadow-[0_0_20px_rgba(14,165,233,0.5)] animate-[spin_25s_linear_infinite_reverse]">
                        ...
                        <div className="absolute inset-8 rounded-full border border-primary/30 animate-[spin_15s_linear_infinite_reverse] hover:[animation-play-state:paused]">
                            {/* Code / React */}
                            <div className="absolute top-1/4 -left-4 w-9 h-9 bg-[#050505] rounded-full border border-primary/50 flex items-center justify-center shadow-[0_0_20px_rgba(20,184,166,0.6)] animate-[spin_15s_linear_infinite]">
                            ...
                            {/* Smartphone / Flutter */}
                            <div className="absolute bottom-1/4 -right-4 w-9 h-9 bg-[#050505] rounded-full border border-primary/50 flex items-center justify-center shadow-[0_0_20px_rgba(20,184,166,0.6)] animate-[spin_15s_linear_infinite]">
                        ...
                        <div className="absolute inset-16 rounded-full border border-purple-500/30 animate-[spin_10s_linear_infinite] hover:[animation-play-state:paused]">
                            {/* Server */}
                            <div className="absolute -top-3 left-1/4 w-8 h-8 bg-[#050505] rounded-full border border-purple-500/60 flex items-center justify-center shadow-[0_0_20px_rgba(168,85,247,0.7)] animate-[spin_10s_linear_infinite_reverse]">
                            ...
                            {/* Database */}
                            <div className="absolute -bottom-3 right-1/4 w-8 h-8 bg-[#050505] rounded-full border border-purple-500/60 flex items-center justify-center shadow-[0_0_20px_rgba(168,85,247,0.7)] animate-[spin_10s_linear_infinite_reverse]">
                        ...
                        <div className="absolute z-10 w-44 h-44 rounded-full glass-panel flex flex-col items-center justify-center gap-3 border-primary/40 shadow-[0_0_40px_rgba(20,184,166,0.25)] hover:shadow-[0_0_60px_rgba(20,184,166,0.5)] hover:scale-105 transition-all duration-500 cursor-pointer group bg-[#050505]/80 backdrop-blur-2xl">
                            ...
                            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-primary via-purple-500 to-secondary p-[2px] group-hover:rotate-180 transition-transform duration-700">
                                <div className="w-full h-full bg-[#050505] rounded-full flex items-center justify-center group-hover:-rotate-180 transition-transform duration-700">
                            ...
                            <div className="flex flex-col items-center gap-1">
                                <div className="flex items-center gap-2 bg-white/5 px-3 py-1 rounded-full border border-white/10">
                                    ...
                                    <span className="text-[10px] font-bold text-green-400 tracking-widest uppercase">System Active</span>
                                </div>
                                <span className="text-[10px] text-slate-400 font-medium tracking-wide">Available for Hire</span>
                            </div>
